...
function Show({ product, variationOptions }:
    ...
    useEffect(() => {
        for (let type of product.variationTypes) {
            const selectdOptionId: number = variationOptions[type.id];
            console.log(selectdOptionId, type.options);
            chooseOption(
                type.id,
                type.options.find(op => op.id === selectdOptionId) || type.options[0],
                false
            )
        }
    }, []);
                        ...
                        <div className="flex gap-2 mb-4">
                            {type.options.map(option => (
                                <div onClick={() => chooseOption(type.id, option)} key={option.id}>
                                    {option.images && <img src={option.images[0].thumb} alt="" className={"w-[50px]" + (option.id === selectedOptions[type.id]?.id ? 'outline outline-4 outline-primary' : '')} />}
                                </div>
                            ))}
                        ...
                        <div className="flex join mb-4">
                            {type.options.map(option => (
                                <input onChange={() => chooseOption(type.id, option)}
                                    ...
                                    checked={option.id === selectedOptions[type.id]?.id}
